...
def _entry_price_metrics(result):
    seat = _entry_votes(result).get("S1_cross_venue_price_acceptance") or {}
    return seat.get("metrics") or {}


def _entry_flow_metrics(result):
    seat = _entry_votes(result).get("S2_multi_venue_executed_flow") or {}
    return seat.get("metrics") or {}
...
def fast_contract_ok(result):
    if not result or result.get("decision") != "GO" or result.get("entry_mode") != "FAST":
        return False
    pm, fm = _entry_price_metrics(result), _entry_flow_metrics(result)
    return (
...
def classify(result, state):
    ...
    multiple = expected_bps / TOTAL_COST_BPS if TOTAL_COST_BPS > 0 else 999.0
