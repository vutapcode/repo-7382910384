...
VERSION = "REGIME_OI_FRESHNESS_HOOK_V4_POLL_AWARE"
MIN_MAX_OI_AGE_S = 8.0
MAX_MAX_OI_AGE_S = 18.0
...
def oi_poll_interval_seconds(state):
    return _f(
        getattr(state, "oi_poll_interval_effective_seconds", 0.0)
        or getattr(state, "oi_poll_interval_seconds", 0.0)
        or 15.0
    )


def max_oi_age_seconds(state):
    """Allow one scheduled OI poll plus bounded transport jitter."""
    interval = oi_poll_interval_seconds(state)
    return max(MIN_MAX_OI_AGE_S, min(MAX_MAX_OI_AGE_S, interval + 3.0))
...
def install(regime_module):
    ...
    def classify(state, side=None):
        ...
        max_age = max_oi_age_seconds(state)
        stale = age > max_age
        ...
        report["oi_max_age_s"] = round(max_age, 3)
        report["oi_poll_interval_s"] = round(
            max(0.0, oi_poll_interval_seconds(state)), 3
        )
