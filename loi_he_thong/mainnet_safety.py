...
def execution_venue():
    return os.getenv("SMC_EXECUTION_VENUE", "TESTNET").strip().upper()
...
def fixed_quantity():
    return float(os.getenv("SMC_FIXED_QTY_BTC", "0.001"))


def leverage():
    return int(os.getenv("SMC_LEVERAGE", "20"))


def max_entries_per_day():
    return int(os.getenv("SMC_MAX_FILLED_ENTRIES_PER_VN_DAY", "8"))


def daily_loss_limit():
    return abs(float(os.getenv("SMC_DAILY_NET_LOSS_USDT", "0.30")))


def max_planned_loss_usdt():
    return abs(float(os.getenv("SMC_MAX_PLANNED_LOSS_USDT", "0.12")))


def max_consecutive_losses():
    return int(os.getenv("SMC_MAX_CONSECUTIVE_LOSSES", "2"))
...
def reserve_usdt():
    return float(os.getenv("SMC_MAINNET_MARGIN_RESERVE_USDT", "0.50"))


def _safety_state_path(state=None):
    configured = os.getenv("SMC_MAINNET_SAFETY_STATE_PATH", "").strip()
    if configured:
...
def validate_static_config(filters):
    ...
    if os.getenv("SMC_MARGIN_TYPE", "ISOLATED").upper() != "ISOLATED":
...
async def refresh_daily_gate(api, state, now=None):
    ...
    for order in orders if isinstance(orders, list) else ():
        ...
        if executed > 0.0 and client_id.startswith(("smc_entry_", "smc_passi_")):
    ...
    if len(entry_ids) >= max_entries_per_day():
...
async def prepare_mainnet_account(api, state):
    """Configure account, and only when armed flatten startup residue safely."""
    if not is_mainnet(api):
        return True, "TESTNET"
    if not mainnet_armed():
        return False, "MAINNET_NOT_ARMED"
    ...
    # Dedicated subaccount policy: every startup begins flat. Keep exchange SLs
    # until the reduce-only equivalent close has been confirmed.
    ...
    other_active = [
        ...
        and abs(float(row.get("positionAmt", 0.0) or 0.0)) > 0.0
    ]
    other_orders = [
        row for row in list(all_orders) + list(all_algos)
        if str(row.get("symbol", SYMBOL)) != SYMBOL
    ]
    if other_active or other_orders:
        return False, "MAINNET_EXCLUSIVE_ACCOUNT_CONTAMINATED"
    _, cancel_status = await api.cancel_all_open_orders(SYMBOL)
    if cancel_status != 200:
        return False, "MAINNET_STARTUP_CANCEL_ORDERS_FAILED"
    ...
    for row in positions:
        ...
        if abs(amount) <= 0.0:
            continue
        side = row.get("positionSide") or ("LONG" if amount > 0.0 else "SHORT")
        client_id = startup_client_id(getattr(state, "run_id", ""), side)
        result, close_status = await api.new_order(
            SYMBOL, "SELL" if side == "LONG" else "BUY", "MARKET", abs(amount),
            positionSide=side, newOrderRespType="RESULT", newClientOrderId=client_id,
        )
        if close_status not in (200, 599):
            return False, "MAINNET_STARTUP_FLATTEN_FAILED"
        if close_status == 599:
            result, close_status = await api.query_order(SYMBOL, client_id)
            if close_status != 200:
                return False, "MAINNET_STARTUP_FLATTEN_UNKNOWN"
    positions, status = await api.get_positions(SYMBOL)
    if status != 200 or any(
        abs(float(row.get("positionAmt", 0.0) or 0.0)) > 0.0 for row in positions
    ):
        return False, "MAINNET_STARTUP_NOT_FLAT"
    _, cancel_algo_status = await api.cancel_all_algo_orders(SYMBOL)
    if cancel_algo_status != 200:
        return False, "MAINNET_STARTUP_CANCEL_ALGOS_FAILED"
    algos, algo_status = await api.get_open_algo_orders(SYMBOL)
    if algo_status != 200 or algos:
        return False, "MAINNET_STARTUP_ALGO_STATE_UNVERIFIED"
    return True, "PASS"
