...
Legacy modules stay available for tests/replay, but production does not spawn
their disabled loops. This keeps runtime authority explicit and trims idle/no-op
work from the hot path.
"""
import asyncio
import logging

VERSION = "TIER_S_RUNTIME_PRUNE_V1"
...
async def _lean_main(app):
    ...
    task_specs = (
        ...
        ("executionBookTicker", lambda: app.tai_gia_tick.hung_gia_tick_execution("btcusdt", state, testnet=app.api.testnet)),
        ("spot_cvd", lambda: _spot_flow_loop(app)),
        ("vi_mo_input", lambda: app.tai_vi_mo.tai_du_lieu_vi_mo("BTCUSDT", state)),
        ("M1_atr", app.vong_lap_nen_m1),
