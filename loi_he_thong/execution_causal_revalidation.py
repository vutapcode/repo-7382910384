...
def _rows_after(state, result):
    """Return finalized no-lookahead rows strictly newer than the decision."""
    engine = _engine(state)
    if engine is None:
        return None
    cutoff = int(_f((result or {}).get("ts")) * 1000.0)
    return {
        name: tuple(
            row for row in venue.history
            if int(row.get("receive_time_ms", 0) or 0) > cutoff
        )
...
def _current_release(state, side, result):
...
def maker_ttl_release(state, side, result, now):
    ...
    if not ok:
        return ok, reason, detail
    ok, reason, detail = _current_release(state, side, result)
    if not ok:
        return ok, reason, detail
