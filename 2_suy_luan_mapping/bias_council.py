...
def _a2(cur,ref,t):
 ...
 ratio=abs(s)/max(abs(f),t*.25);return _v("LONG" if s>0 else "SHORT",_c(.45+.1*min(max(ratio-1,0),2)),"SPOT_LEADS_PERP",lead_ratio=ratio)

def _cons(sv,av,story):
 L=[(k,v) for k,v in sv.items() if v["vote"]=="LONG"];S=[(k,v) for k,v in sv.items() if v["vote"]=="SHORT"]
 def score(side):return sum(W[k]*v["confidence"] for k,v in sv.items() if v["vote"]==side)+sum(.12*v["confidence"] for v in av.values() if v["vote"]==side)
 ls,ss=score("LONG"),score("SHORT");name,sd,bonus,veto=story
 if veto:return "ABSTAIN",0.,0,"CAUSAL_STORY_VETO",ls,ss
 if len(L)>=2:side,sup,opp="LONG",L,S
 elif len(S)>=2:side,sup,opp="SHORT",S,L
 else:return "ABSTAIN",0.,max(len(L),len(S)),"INSUFFICIENT_S_QUORUM",ls,ss
 if sd not in ("ABSTAIN",side):return "ABSTAIN",0.,len(sup),"STORY_DIRECTION_CONFLICT",ls,ss
 ...
 base=sum(W[k]*v["confidence"] for k,v in sup)/sum(W[k] for k,_ in sup)+(.06 if len(sup)==3 else 0)-sum(.10*v["confidence"] for _,v in opp)
 for v in av.values():base+=(.035 if v["vote"]==side else -.035 if v["vote"] in ("LONG","SHORT") else 0)*v["confidence"]
 base+=bonus;tot=ls+ss;base+=min(.045,(abs(ls-ss)/tot if tot>0 else 0)*.045)
 return side,_c(base),len(sup),name,ls,ss

def evaluate(s,now=None,force_full=False):
 now=time.time() if now is None else float(now)
 st=float(getattr(s,"thoi_gian_tick_cuoi",0) or 0);sf=_fresh(st,now,SPOT_AGE);spot=_mid(getattr(s,"best_bid",0),getattr(s,"best_ask",0)) if sf else 0.
 ct=float(getattr(s,"thoi_gian_coinbase_ticker_cuoi",0) or 0) or float(getattr(s,"thoi_gian_coinbase_cuoi",0) or 0);cb=float(getattr(s,"coinbase_price",0) or 0) if _fresh(ct,now,CB_AGE) else 0.
 fut,src=_fut(s,now);mt=float(getattr(s,"thoi_gian_vi_mo_cuoi",0) or 0);mf=_fresh(mt,now,OI_AGE);oi=float(getattr(s,"open_interest",0) or 0) if mf else 0.
 ...
 slow,fast=_ref(h,now-LOOKBACK,3.),_ref(h,now-FAST,1.25);cur={"ts":now,"spot":spot,"coinbase":cb,"futures":fut,"oi":oi};h.append(cur);t=_thr(s,spot)
 sv={"S1_cross_price":_s1(cur,slow,fast,t),"S2_price_x_oi":_s2(cur,slow,t,mf),"S3_multi_flow":_s3(s,now)};story=_story(sv)
 av={"A1_funding_basis":_a1(s,spot,fut,mf),"A2_spot_lead":_a2(cur,slow,t)}
 bias,conf,q,reason,ls,ss=_cons(sv,av,story);tot=ls+ss
 return {"version":VERSION,"bias":bias,"confidence":round(conf,6),"quorum":q,"reason":reason,"mode":"FULL","story":{"name":story[0],"direction":story[1],"confidence_adjustment":story[2],"veto":story[3]},"s_votes":sv,"a_votes":av,"direction_scores":{"long":round(ls,6),"short":round(ss,6),"margin":round(_c(abs(ls-ss)/tot if tot>0 else 0),6)},"freshness":{"spot":sf,"coinbase":_fresh(ct,now,CB_AGE),"futures":fut>0,"oi_macro":mf},"contract":"DIRECTION_ONLY_NO_ENTRY_TIMING","futures_price_source":src,"ts":now}

def _hyst(s,r):
 now=float(r["ts"]);new=r["bias"];nc=float(r["confidence"]);old=str(getattr(s,"bias_state","ABSTAIN") or "ABSTAIN").upper();oc=float(getattr(s,"bias_confidence",0) or 0);cand=str(getattr(s,"_bias_flip_candidate","") or "");since=float(getattr(s,"_bias_flip_since",0) or 0);last=float(getattr(s,"_bias_last_supported_at",0) or 0)
 ...
 if new==old:s._bias_flip_candidate="";s._bias_flip_since=0.;s._bias_last_supported_at=now;return old,nc,"STABLE"
 if new=="ABSTAIN":
  s._bias_flip_candidate="";s._bias_flip_since=0.
  return (old,oc*.82,"HOLD_THROUGH_ABSTAIN") if last>0 and now-last<=H_ABS else ("ABSTAIN",0.,"RELEASE_TO_ABSTAIN")
 story=(r.get("story") or {}).get("name","");confirmed=story in ("NEW_LONG_BUILD_CONFIRMED","NEW_SHORT_BUILD_CONFIRMED")
 if int(r.get("quorum",0) or 0)>=3 and nc>=H_FAST and confirmed:s._bias_flip_candidate="";s._bias_flip_since=0.;s._bias_last_supported_at=now;return new,nc,"FAST_CONFIRMED_FLIP"
 if cand!=new or since<=0:s._bias_flip_candidate=new;s._bias_flip_since=now;return old,oc*.72,"PENDING_FLIP"
 if now-since>=H_FLIP and nc>=.58:s._bias_flip_candidate="";s._bias_flip_since=0.;s._bias_last_supported_at=now;return new,nc,"CONFIRMED_FLIP"
 return old,oc*.72,"PENDING_FLIP"

def update_state(s,now=None,force_full=False):
 raw=evaluate(s,now,force_full);side,conf,h=_hyst(s,raw);out=dict(raw);out.update(raw_bias=raw["bias"],raw_confidence=raw["confidence"],bias=side,confidence=round(_c(conf),6),hysteresis=h)
 if side in ("LONG","SHORT") and raw["bias"]==side:s._bias_last_supported_at=float(out["ts"])
 s.bias_state=side;s.bias_confidence=out["confidence"];s.bias_council=out;s.bias_updated_at=out["ts"];s.bias_version=VERSION;s.macro_bias="NEUTRAL";return out
