...
class LocalOrderBook:
    ...
    def apply_partial(self, event):
        """Replace the book from a Binance partial-depth top-N event."""
        final_u = int(event['u'])
        previous_u = int(event.get('pu', 0) or 0)
        if self.last_u is not None and previous_u and previous_u != self.last_u:
            raise DepthGap(
                f'PU_GAP expected={self.last_u} actual={previous_u} '
                f'U={event.get("U")} u={final_u}'
            )
        bids = event.get('b', []) or []
        asks = event.get('a', []) or []
        if not bids or not asks:
            raise DepthGap('PARTIAL_DEPTH_EMPTY_SIDE')
        self.bids = {str(price): str(qty) for price, qty in bids[:20]}
        self.asks = {str(price): str(qty) for price, qty in asks[:20]}
        if self.snapshot_update_id is None:
            self.snapshot_update_id = final_u
        self.last_u = final_u
        self.synced = True
        return 'APPLIED'
