...
class MainnetSafetyTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_prepare_refuses_existing_position_without_flattening(self):
        api = FakeAPI()
        api.positions[0]['positionAmt'] = '0.001'
        api.new_order = mock.AsyncMock()
        state = SimpleNamespace(run_id='run-test')
        with self.env():
            ok, reason = await safety.prepare_mainnet_account(api, state)
        self.assertFalse(ok)
        self.assertEqual(reason, 'MAINNET_ACCOUNT_NOT_FLAT')
        api.new_order.assert_not_awaited()
