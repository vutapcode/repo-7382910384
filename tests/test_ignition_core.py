...
class IgnitionCoreTests(unittest.TestCase):
    ...
    def test_acquisition_handoff_fails_closed_across_epoch_or_cash_stop(self):
        ...
        for row in stopped["binance_spot"] + stopped["coinbase_spot"]:
            row["price_conversion_bps"] = 0.01
        self.assertIsNone(ignition_core._start_acquisition_handoff_episode(
    ...
    def test_sealed_acquisition_rejects_current_opposite_cash(self):
        ...
        s.bias_acquisition_handoff = acquisition_handoff()
        histories = self._acquisition_histories()
        opposing = evidence_row(99_700, "SHORT", 99.998)
        opposing["venue"] = "coinbase_spot"
        histories["coinbase_spot"] = (opposing,)
        ...
        self.assertEqual(
            s._ignition_acquisition_handoff_observation["status"],
            "ACQUISITION_CURRENT_CASH_CONTRADICTION",
        )
    ...
    def test_dual_cash_failed_continuation_promotes_without_bias_flip(self):
        ...
        self.assertTrue(
            transition["proof_nodes"]["old_side_failure"]["proved"]
        )
        self.assertTrue(
            transition["proof_nodes"]["derivative_pressure"]["observed"]
        )
        self.assertIn(
            transition["control_phase"],
            {"NEW_SIDE_ACCEPTED", "NEW_SIDE_CONTINUING", "NEW_SIDE_FADING"},
        )
        self.assertEqual(promoted["bias_snapshot"]["direction"], "SHORT")
        self.assertEqual(promoted["side"], "LONG")

    def test_proved_old_side_failure_survives_new_side_evidence_lull(self):
        s = state(now=2.5)
        pending = {
            "episode_id": "ign:futures:LONG:1000",
            "episode_hash": "immutable-hash",
            "causal_episode_id": "ign:futures:LONG:1000",
            "side": "LONG",
            "started_receive_ms": 1_000,
            "last_evidence_ms": 1_200,
            "epochs": {},
        ...
        with patch.object(
            ...
            result = ignition_core._resolve_pending_reversal(
                s, {}, 2_500, allow_promotion=True,
            )
        ...
        self.assertEqual(
            s._ignition_last_reject,
            "PENDING_REVERSAL_CONTROL_TRANSFER_WAITING_FRESH_CASH",
        )

    def test_unproved_transition_still_expires_on_evidence_gap(self):
        s = state(now=2.5)
        s._ignition_pending_reversal_episode = {
            "episode_id": "ign:futures:LONG:1000",
            "episode_hash": "immutable-hash",
            "causal_episode_id": "ign:futures:LONG:1000",
            "side": "LONG",
        ...
        transition = {
            ...
            "control_transfer_memory_valid": False,
        }
        with patch.object(
            ignition_core, "_transition_snapshot", return_value=transition,
        ):
        ...
        self.assertEqual(
            s._ignition_last_reject, "PENDING_REVERSAL_EVIDENCE_GAP"
        )

    def test_fresh_cash_resumes_same_control_transfer_episode(self):
        s = state(now=2.7)
        pending = {
            "episode_id": "ign:futures:LONG:1000",
            "episode_hash": "immutable-hash",
        ...
        with patch.object(
            ...
            promoted = ignition_core._resolve_pending_reversal(
                s, {}, 2_700, allow_promotion=True,
            )

        self.assertIs(promoted, pending)
        self.assertEqual(promoted["episode_hash"], "immutable-hash")
        self.assertTrue(promoted["transition_confirmed"])
    ...
    def test_perp_lead_is_context_after_cash_proposer(self):
        s = state()
        result = {
            "decision": "GO", "side": "LONG", "entry_mode": "IGNITION",
        ...
        self.assertNotIn("PERP_LED_VETO", report["hard_vetoes"])
        self.assertIn("PERP_LED_CONTEXT", report["diagnostic_flags"])
        independence = report["entry_thesis_audit"]["questions"][
            "q6_cross_venue_corroboration"
        ]
        self.assertTrue(independence["perp_expansion_context"])
        self.assertNotEqual(independence["status"], "DERIVATIVES_LED_REJECT")
