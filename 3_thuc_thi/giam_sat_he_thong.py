...
FEEDS = {
    ...
    'OI/Funding': ('thoi_gian_vi_mo_cuoi', 18.0),
...
def readiness(state):
    ...
    # Mainnet shadow owns a smaller, causal-only feed contract. Once its health
    # probe is installed, do not reapply the retired orderbook/POC/reconcile
    # requirements below and race the authoritative entry gate.
    if getattr(state, 'shadow_readiness_authoritative', False):
        shadow_health = getattr(state, 'mainnet_shadow_health', {}) or {}
        if bool(shadow_health.get('entry_ready', False)):
            return True, 'SHADOW_READY'
        return False, getattr(
            state, 'last_readiness_reason', 'SHADOW_FEED_NOT_READY'
        )
